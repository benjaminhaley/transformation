...
# TODO derive tiny from medium and use it for most of this project
...
default <- data.frame(value=values, 
                      count=0, 
                      frequency=0.0002)
pixels <- sample(names(g))
r <- ldply(pixels, function(a) {
  ldply(pixels, function(b) {
    a. <- as.numeric(a)
    b. <- as.numeric(b)
    if(a. <= b.) return(NULL)
...
nclusters <- 100
k <- kmeans(r[,values], nclusters, iter.max=20)
r$cluster <- k$cluster

# Add cluster counts
counts <- r %>% ungroup() %>% group_by(cluster) %>% summarize(n=length(cluster))
r <- merge(r[,!names(r) %in% 'n'], counts, all.x=T)

# Add likelihood
centers <- data.frame(
  loglikelihood = rowSums(k$centers),
  cluster=1:nclusters
)
r <- merge(r[,!names(r) %in% 'loglikelihood'], centers, all.x=T)

ggplot(r) +
  geom_segment(aes(x=x, xend=xend, y=y, yend=yend,
               alpha=1/(n*((xend - x)^2 + (yend - y)^2)^0.5))) +
  facet_wrap(~ loglikelihood + cluster) +
  scale_alpha_continuous(range=c(0.01, .5)) +
  theme_nothing()

# These images (cool.png, cool2.png, cool3.png) are exceedingly cool and intriguing
# but they are not showing the general rules of transformations, instead something
# specific to these images.

# I think I need to start from a humbler place.  What does it like like when we 
# compare the 3 pixels taken before to other sets of 3 in a highly localize region

# Once we understand this point we can move forward.



r$cluster <- kmeans(r[,values], 100)$cluster
